...
export const resolvers = {
  ...
  Query: {
    ...
    // Get teams with bounded location updates for an event in one batched query
    teamsWithUpdates: async (_, { event_id, limit = 100 }) => {
      ...
      const result = await query(
        `WITH ranked_updates AS (
           SELECT
             ...
             l.id AS update_id,
             l.team AS update_team,
             l.event AS update_event,
             l.lat,
             l.lon,
             l.timestamp,
             ROW_NUMBER() OVER (
         ...
         SELECT *
         FROM ranked_updates
         WHERE rn IS NULL OR rn <= $2
         ORDER BY name ASC, rn ASC NULLS LAST, timestamp DESC`,
        [event_id, safeLimit]
      );
      ...
      for (const row of result.rows) {
        ...
        if (row.update_id) {
          teamsById.get(row.team_id)._updates.push({
            id: row.update_id,
            team: row.update_team,
            event: row.update_event,
  ...
  Team: {
    ...
    updates: async (parent, args = {}) => {
      ...
      if (Array.isArray(parent._updates)) {
        return parent._updates.slice(0, limit);
      }
